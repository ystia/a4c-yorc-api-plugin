package alien4cloud.yorc.api.plugin.context.rest;
...
public class TemplateManager implements SelfNaming {
    ...
    public void configure() throws PluginConfigurationException {
        ...
        log.info("Configuring connection manager using ConnectionMaxPoolSize: {}, ConnectionTtl: {}s",
                configuration.getConnectionMaxPoolSize(), configuration.getConnectionTtl());
        log.info("Connection eviction will be done each {} seconds, ConnectionMaxIdleTime: {}s",
                configuration.getConnectionEvictionPeriod(), configuration.getConnectionMaxIdleTime());
                ...
                disposable = Completable.timer(
                    configuration.getConnectionEvictionPeriod(),
                    TimeUnit.SECONDS,
                    scheduler).subscribe(this::evictionTask);
            }
    ...
    public void evictionTask() {
        ...
        disposable = Completable.timer(configuration.getConnectionEvictionPeriod(), TimeUnit.SECONDS, scheduler)
                .subscribe(this::evictionTask);
    }

    public String getYorcUrl() {
        ...
        if(configuration.getUrlYorc().startsWith("https")) {
            ...
            if ((caCertif == null || caCertif.isEmpty())
                ...
                if (System.getProperty("javax.net.ssl.trustStore") == null || System.getProperty("javax.net.ssl.trustStorePassword") == null) {
                    log.warn("You didn't provide client truststore and password. Using default one \n" +
